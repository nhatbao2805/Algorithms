...
function activitySelection(activities) {
  ...
  console.log(`  ✅ Chọn: ${activities[0].name} [${activities[0].start}-${activities[0].end}]`);
  
  for (let i = 1; i < activities.length; i++) {
    if (activities[i].start >= lastEnd) {
      selected.push(activities[i]);
      lastEnd = activities[i].end;
      console.log(`  ✅ Chọn: ${activities[i].name} [${activities[i].start}-${activities[i].end}]`);
    } else {
      console.log(`  ❌ Bỏ:   ${activities[i].name} [${activities[i].start}-${activities[i].end}] (trùng)`);
    }
  }
...
function fractionalKnapsack(items, capacity) {
  ...
  items.forEach(i => {
    ...
    console.log(`  ${i.name}: w=${i.weight}, v=${i.value}, ratio=${i.ratio.toFixed(2)}`);
  });
  ...
  for (const item of items) {
    if (remaining >= item.weight) {
      ...
      console.log(`  Lấy hết ${item.name}: +${item.value} (còn ${remaining}kg)`);
    } else if (remaining > 0) {
      const fraction = remaining / item.weight;
      totalValue += item.value * fraction;
      console.log(`  Lấy ${(fraction * 100).toFixed(0)}% ${item.name}: +${(item.value * fraction).toFixed(1)}`);
      remaining = 0;
    }
  }
