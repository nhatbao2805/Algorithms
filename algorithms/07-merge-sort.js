...
function example3() {
  ...
  console.log(`Số bước tối đa: 20 * log2(20) ≈ ${Math.ceil(20 * Math.log2(20))} phép so sánh`);
}
...
function mergeSortCountInversions(arr) {
  ...
  const sorted = merged.concat(left.sorted.slice(i)).concat(right.sorted.slice(j));
  return { sorted, count };
}
