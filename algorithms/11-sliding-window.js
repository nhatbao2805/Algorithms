...
function maxSumSubarray(arr, k) {
  ...
  for (let i = k; i < arr.length; i++) {
    ...
    console.log(`  Trượt → [${i - k + 1}..${i}]: +${arr[i]} -${arr[i - k]} = ${windowSum}`);
...
function findAnagrams(str, pattern) {
  ...
  for (let right = 0; right < str.length; right++) {
    ...
    if (patternMap[rightChar] && windowMap[rightChar] === patternMap[rightChar]) matched++;
    
    if (right >= pattern.length) {
      const leftChar = str[right - pattern.length];
      if (patternMap[leftChar] && windowMap[leftChar] === patternMap[leftChar]) matched--;
      windowMap[leftChar]--;
    }
    ...
    if (matched === needed) {
      ...
      console.log(`  ✅ Anagram tại index ${startIdx}: "${str.slice(startIdx, startIdx + pattern.length)}"`);
    }
  }
