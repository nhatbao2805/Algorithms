...
function selectionSortDebug(arr) {
  ...
  for (let i = 0; i < result.length - 1; i++) {
    ...
    console.log(`  Lượt ${i + 1}: Tìm min trong [${result.slice(i)}] → min=${result[minIdx]} tại index ${minIdx}`);
    if (minIdx !== i) {
      [result[i], result[minIdx]] = [result[minIdx], result[i]];
      swapCount++;
...
function example2() {
  ...
  console.log(`Bubble Sort: Có thể lên đến ${arr.length * (arr.length - 1) / 2} lần swap`);
