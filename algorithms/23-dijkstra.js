  ...
  while (visited.size < Object.keys(graph).length) {
    ...
    for (const [neighbor, weight] of Object.entries(graph[minNode] || {})) {
      ...
      if (newDist < dist[neighbor]) {
        console.log(`    ${neighbor}: ${dist[neighbor]} → ${newDist} (qua ${minNode})`);
...
function networkDelay(times, n, k) {
  ...
  const maxDelay = Math.max(...Object.values(dist).filter(d => d !== Infinity));
...
function example3() {
  ...
  console.log('━'.repeat(50));
  // [[from, to, weight], ...]
  networkDelay([[2,1,1],[2,3,1],[3,4,1]], 4, 2);
}
...
function findCheapestPrice(n, flights, src, dst, k) {
  ...
  for (let i = 0; i <= k; i++) {
    ...
    dist = temp;
    console.log(`  Stop ${i}: dist=[${dist.map(d => d === Infinity ? '∞' : d)}]`);
  }
...
function example4() {
  ...
  findCheapestPrice(4, [[0,1,100],[1,2,100],[0,2,500],[2,3,100]], 0, 3, 1);
}
