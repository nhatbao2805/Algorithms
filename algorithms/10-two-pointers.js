...
function twoSumSorted(arr, target) {
  ...
  while (left < right) {
    ...
    console.log(`  left=${left}(${arr[left]}) + right=${right}(${arr[right]}) = ${sum}`);
...
function removeDuplicates(arr) {
  ...
  console.log(`  → Mảng unique: [${arr.slice(0, slow + 1)}], length = ${slow + 1}`);
  return slow + 1;
}
...
function maxArea(heights) {
  ...
  while (left < right) {
    ...
    console.log(`  left=${left}(h=${heights[left]}), right=${right}(h=${heights[right]}), area=${area}, max=${maxWater}`);
