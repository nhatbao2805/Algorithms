# Ôn Tập Mã Nguồn — `algorithms/15-hash-table.js`
...
function example5() {
  ...
  const ops = [
    () => {
      lru.put(1, "A");
      console.log(
        `  put(1, 'A') → cache: [${[...lru.cache.entries()].map((e) => `${e[0]}:${e[1]}`)}]`,
      );
    },
    () => {
      lru.put(2, "B");
      console.log(
        `  put(2, 'B') → cache: [${[...lru.cache.entries()].map((e) => `${e[0]}:${e[1]}`)}]`,
      );
    },
    () => {
      lru.put(3, "C");
      console.log(
        `  put(3, 'C') → cache: [${[...lru.cache.entries()].map((e) => `${e[0]}:${e[1]}`)}]`,
      );
    },
    () => {
      console.log(
        `  get(1): ${lru.get(1)} → cache: [${[...lru.cache.entries()].map((e) => `${e[0]}:${e[1]}`)}]`,
      );
    },
    () => {
      lru.put(4, "D");
      console.log(
        `  put(4, 'D') → cache: [${[...lru.cache.entries()].map((e) => `${e[0]}:${e[1]}`)}] (evicted key 2!)`,
      );
    },
    () => {
