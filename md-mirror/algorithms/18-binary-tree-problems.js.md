# Ôn Tập Mã Nguồn — `algorithms/18-binary-tree-problems.js`
...
- **Cơ bản**
  ...
  - BÀI 7: Đếm số node trong cây (countNodes).
  - BÀI 8: Tìm node có giá trị lớn nhất / nhỏ nhất trong cây (BST: min=leftmost, max=rightmost).
- **Trung bình**
  - BÀI 9: Binary tree từ preorder và inorder (xây lại cây, root = pre[0], tìm trong inorder).
  - BÀI 10: Kth smallest in BST (inorder đến phần tử thứ k).
  - BÀI 11: Serialize / Deserialize binary tree (chuỗi hóa và khôi phục cây).
- **Nâng cao**
  - BÀI 12: Binary tree maximum path sum: đường đi bất kỳ (node có thể dùng tối đa 1 lần), trả về tổng lớn nhất.
  - BÀI 13: Lowest Common Ancestor (cây nhị phân thường, không phải BST).
  - BÀI 14: Recover BST: 2 node bị swap sai, tìm và sửa (inorder tìm 2 vị trí nghịch thế).
