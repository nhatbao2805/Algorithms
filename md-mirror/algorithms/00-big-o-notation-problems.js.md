# Ôn Tập Mã Nguồn — `algorithms/00-big-o-notation-problems.js`
...
- **Cơ bản**
  ...
  - BÀI 5: Phân tích O(?) của 2 vòng for lồng nhau, mỗi vòng n lần.
  - BÀI 6: Cho đoạn code: tìm min trong mảng rồi tìm max. Time? Space?
- **Trung bình**
  - BÀI 7: Đếm số cặp (i,j) với a[i] + a[j] = target. Cách O(n²) vs O(n) dùng map.
  - BÀI 8: Cho thuật toán đệ quy fib(n): phân tích time/space, so với fibMemo(n).
  - BÀI 9: So sánh O(n), O(n log n), O(n²) bằng đo thời gian với n = 1000, 10000, 50000.
- **Nâng cao**
  - BÀI 10: Thiết kế thuật toán tìm 3 phần tử có tổng = 0, phân tích O(?) và tối ưu.
  - BÀI 11: Phân tích space complexity của merge sort (call stack + mảng tạm).
  - BÀI 12: Cho code có vòng lặp while bên trong for: n lần, mỗi lần while tối đa k bước. Tổng O(?) khi k phụ thuộc n.
