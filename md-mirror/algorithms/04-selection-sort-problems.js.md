# Ôn Tập Mã Nguồn — `algorithms/04-selection-sort-problems.js`
...
- **Cơ bản**
  ...
  - BÀI 5: Selection sort cho mảng string (so sánh theo thứ tự từ điển).
  - BÀI 6: Trả về số lần hoán đổi (swap) đã thực hiện.
- **Trung bình**
  - BÀI 7: Dual selection sort: mỗi pass vừa chọn min đưa về đầu vừa chọn max đưa về cuối.
  - BÀI 8: Sắp xếp mảng object theo key: selectionSort(arr, key) với key='age' hoặc 'name'.
  - BÀI 9: So sánh số lần so sánh (comparison) của selection vs bubble (cùng mảng).
- **Nâng cao**
  - BÀI 10: Chứng minh selection sort không ổn định (cho ví dụ cụ thể).
  - BÀI 11: Heap sort dựa trên ý tưởng 'chọn max' nhưng dùng heap → O(n log n). So sánh với selection.
  - BÀI 12: Tournament sort (tree selection): dùng cây để tìm min nhanh hơn cho từng pass.
