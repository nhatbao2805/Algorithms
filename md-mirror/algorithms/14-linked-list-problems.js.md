# Ôn Tập Mã Nguồn — `algorithms/14-linked-list-problems.js`
...
- **Cơ bản**
  ...
  - BÀI 7: Tìm node giữa (middle): nếu chẵn trả node thứ n/2 (slow/fast pointer).
  - BÀI 8: Xoá node khi chỉ có con trỏ tới chính node đó (không có head). Gợi ý: copy value từ next rồi xoá next.
- **Trung bình**
  - BÀI 9: Reverse linked list từ vị trí left đến right (1-indexed).
  - BÀI 10: Swap nodes in pairs: 1→2→3→4 thành 2→1→4→3.
  - BÀI 11: Add two numbers: 2 linked list biểu diễn số (chữ số ngược), trả về list tổng. (2→4→3) + (5→6→4) = (7→0→8).
- **Nâng cao**
  - BÀI 12: Merge K sorted lists (dùng heap hoặc merge lần lượt).
  - BÀI 13: Reorder list: L0→Ln→L1→Ln-1→... (tìm giữa, đảo nửa sau, merge 2 nửa).
  - BÀI 14: Copy list with random pointer: mỗi node có next và random, clone toàn bộ list.
