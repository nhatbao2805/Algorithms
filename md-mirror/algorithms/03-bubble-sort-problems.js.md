# Ôn Tập Mã Nguồn — `algorithms/03-bubble-sort-problems.js`
...
- **Cơ bản**
  ...
  - BÀI 5: In ra mảng sau mỗi pass để quan sát từng bước.
  - BÀI 6: Sắp xếp mảng chuỗi theo thứ tự alphabet (so sánh string).
- **Trung bình**
  - BÀI 7: Bubble sort ổn định? Giải thích và kiểm tra với mảng có (value, id).
  - BÀI 8: Cocktail shaker sort: bubble nhưng đổi chiều mỗi pass. So sánh số lần so sánh.
  - BÀI 9: Sắp xếp mảng object theo nhiều tiêu chí (vd: theo name, rồi theo age).
- **Nâng cao**
  - BÀI 10: Đếm số inversion bằng bubble sort (mỗi swap giảm 1 inversion).
  - BÀI 11: So sánh bubble vs selection vs insertion trên mảng ngẫu nhiên, đã sort, sort ngược (đo thời gian).
  - BÀI 12: Bubble sort với flag 'last swap index': chỉ duyệt đến vị trí swap cuối cùng ở pass trước.
