# Ôn Tập Mã Nguồn — `algorithms/25-event-loop-callstack-problems.js`
...
- **Cơ bản**
  - BÀI 1: Thứ tự sync → Promise.then → setTimeout(0).
  - BÀI 2: Nhiều Promise.then liên tiếp, thứ tự microtask.
- **Trung bình**
  - BÀI 3: Mix microtask + macrotask (setTimeout bên trong có Promise.then).
  - BÀI 4: queueMicrotask vs Promise.then vs setTimeout.
  - BÀI 5: Promise lồng nhau (then tạo thêm then bên trong).
  - BÀI 6: setTimeout được đăng ký bên trong Promise.then.
  - BÀI 7: Microtask starvation (queueMicrotask gọi đệ quy).
- **Nâng cao**
  - BÀI 8:  async/await + Promise + setTimeout.
  - BÀI 9:  Then chain phức tạp + return Promise.resolve().then().
  - BÀI 10: setTimeout lồng nhau + microtask trong từng callback.
  - BÀI 11: async/await lồng nhau (inner async, await p).
  - BÀI 12: Promise.reject + catch + then sau catch.
  - BÀI 13: Promise.all + setTimeout (thứ tự ALL done).
  - BÀI 14: Vòng lặp sync nặng + microtask + macrotask.
  - BÀI 15: Tổng hợp: nhiều microtask, 2 setTimeout, await.
